...
public class Day {
	...
	public int calculateDay() {
		// 1年の日数
    	final int YEAR_DATE = 365;
    	// 月ごとの日数を格納する配列
    	int[] dateString = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    	// うるう年の2月の日数
    	final int LEAP_MONTH_DATE = 29;
    	...
    	// 年単位の日数を求める
    	dayValue += (YEAR_DATE * yearValue) + yearValue/4 - yearValue/100 + yearValue/400;
    	// 月単位の日数を求める
    	// 1月から前の月までの数だけ繰り返す
    	for(int countMonth = 0; countMonth < monthNumber-1; countMonth++) {
    		...
    		if (isLeap(yearNumber) && countMonth == 1) {
    			// うるう年の2月の日数分加算する
    			dayValue += LEAP_MONTH_DATE;
    		// 上記に当てはまらないとき
    		} else {
    			// dateString分加算する
    			dayValue += dateString[countMonth];
    		}
    	}
    	// 現在の日を加算する
    	dayValue += dateNumber;
